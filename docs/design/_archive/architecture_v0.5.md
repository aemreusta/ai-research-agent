# AI Research Agent — Mimari Taslak v0.5

> Durum: **Taslak / tartışma için** · Tarih: 2026-09-10
...
| Frontend | Öneri: API'nin servis ettiği statik HTML + vanilla JS, SSE ile canlı akış (build adımı yok) | öneri D17 |
...
| `api` | Python imajı (`python:3.12-slim` tabanlı) | REST, SSE, UI statik dosyaları, config doğrulama, run oluşturma | default |
...
| 5 | `process_results` | — | URL canonicalization + dedup, snippet triage, top-N tam içerik, near-dup → `origin_cluster` | Fetch hatası → snippet ile devam |
...
| 8 | `cluster_and_corroborate` | — (embedding) | Aynı-olgu kümeleri; `support = distinct origins`; confidence | Lexical fallback |
...
| 13 | `verify_citations` | fast | Cümle ⊨ atıflı claim'ler? Başarısızsa 1 kez geri bildirimle yeniden sentez | — |
...
1. **Hard limits (her zaman geçerli):** `max_iterations=4` (1 ilk + 3 follow-up), `max_searches=30`, `max_cost_usd`, `max_wall_clock=300s` → `stop_reason=budget`
...
| G4 | **Sayısal tutarlılık:** cümledeki her sayı / tarih / yüzde / para tutarı (normalize: `1.2M` = `1,2 milyon`, `%15` = `15%`) atıflı claim'lerin metninde ya da alıntısında geçiyor | error | Cümle çıkarılır, trace'e "unsupported number" |
...
### 15.2 UI (tek sayfa, en basit hali)
...
- **Fiyatlar:** token fiyatları `models.yaml`'da. Her LLM çağrısı maliyeti `llm_calls` tablosuna ve run toplamına yazılır, UI'da canlı gösterilir.
...
## 21. Kapsam ve risk (teslim: Pzt 14 Eyl 16:00)

Değerlendirme ağırlığının %75'i agent kalitesinde (orchestration 25, search 20, reliability 15, prompt 15). Altyapı "Software Architecture" (15) içinde değerlendiriliyor. Bu yüzden altyapı agent'ın zamanını yememeli.
...
> ⚠️ Go dispatcher yaklaşık +0,5–1 gün iş. Azaltma: dispatcher Cuma günü dummy agent'la uçtan uca bitirilir, sonra dokunulmaz. Agent çekirdeği CLI üzerinden paralel geliştirilir.
