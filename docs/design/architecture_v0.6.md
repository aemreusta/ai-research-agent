...
| §8, §10 (canlı run'lardan, D43) | L2 = MinHash; aday = aynı normalize entity + attribute | **L2b**: ≥2 ortak uzun alıntı (≥12 kelime) paylaşan sayfalar tek origin; bir metin tek origin sayılır. Aday kuralı: entity yazım varyantları, attribute eşanlamlıları, birim/ölçek duyarlı değer, **farklı dönem → aday değil**; hakem `consistent` sınıfı; hakem/sentez kaynakların en yeni tarihini görür, değişmiş tarihte güncel taraf tercih edilir |
| §20.2 (D43) | Langfuse sürümü → seed fallback | Langfuse sürümü `seed_version` taşır; repo seed sürümü daha yeniyse yeni seed yayınlanır ve kullanılır (Langfuse'taki düzenlemeler de seed artışında ezilir) |
| §20 (D26) | DSPy/GEPA offline optimize job'ı, `POST /api/optimize`, `optimize/` | **Uygulanmadı.** Prompt'lar sürümlü YAML seed + Langfuse; iyileştirmeler canlı run bulgularından elle yapıldı (D43–D50) |
| §11.2 (D45, D50) | G1–G11 | **G12** (doğrulanmamış/güncelliği teyitsiz bulguya atıf çıkarılır); G10 Türkçe raporda Türkçe harf kontrolü de yapar |
...
Doğrulama durumu (2026-09-17 güncel): Python 543 test (Postgres entegrasyon ve çevrimdışı tam graf senaryoları dahil), Go testleri, `mypy --strict`, compose içinde uçtan uca altyapı akışı (kuyruk → dispatcher → agent → SSE), `--scale agent=3`, agent `kill -9` sonrası requeue + resume, Langfuse headless init ve prompt seed, Presidio maskeleme. Gerçek API'lere **geçersiz** anahtarlarla gidilerek hata yolları doğrulandı (Gemini 400 `API_KEY_INVALID`, Brave 422, OpenAI/Tavily 401 → run 6 sn'de `LLM_AUTH` ile anlaşılır mesajla biter; run başına devre kesici). Geçerli anahtarlarla (Gemini + Tavily) dört örnek soru API üzerinden koşuldu; bulgular ve düzeltmeler `examples/ANALYSIS.md` ve D43'te. 17 Eylül'deki sekiz soruluk tekrarlar, model seçimi ve hukuk tekrarları `examples/2026-09-17-RESULTS.md` ve `docs/review/evaluation_v3.md`'de; GitHub Actions `Verification` iş akışı anahtarsız olarak aynı kontrolleri çalıştırır.
