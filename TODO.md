...
- [~] Uçtan uca "boş run" **lokal süreçlerle doğrulandı**: api → PG → dispatcher → agent (`EchoGraphRunner`) → tek zaman çizelgesi; agent `kill -9` → 30 sn → `AGENT_HEARTBEAT_LOST` → 5 sn backoff → ikinci agent 2. denemede bitirdi, deadline korundu. Kalan: aynısını compose içinde
