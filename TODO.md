...
| D43 | Canlı run bulguları (2026-09-16) | Gerçek anahtarlarla 4 örnek run'dan: (1) aynı basın bülteni farklı sayfa şablonlarıyla MinHash'i geçti → **L2b** ortak alıntı birleştirmesi (`syndication_min_shared_quotes=2`, `_min_quote_words=12`); (2) bir metin konuşmacıyı bazen andığı için iki origin sayılıyordu → metin başına tek origin; (3) hakem çiftlerinin 8/10'u "2025 vs 2030" idi, aynı yılın 6,15 mr / 8.624 mn değerleri hiç karşılaştırılmadı → dönem ön-filtresi, entity/attribute normalizasyonu, birimli değer; (4) KVKK'da iki uyumlu ifade "çelişki" raporlandı → hakeme `consistent` sınıfı; (5) ikinci turda EU AI Act raporu ertelenmiş Annex III tarihini (2 Ağu 2026) güncel gibi yazdı, hakem çifti `different_scope` saydı → hakem ve sentez her bulgunun en yeni kaynak tarihini görür, `different_time` + ledger destekli tercih = güncel konum (`judge_contradictions` v3, `synthesize` v2); (6) şablonu aynı kalan seed güncellemesi Langfuse'a hiç ulaşmıyordu → Langfuse sürümü `seed_version` taşır, repo seed'i daha yeniyse yayınlanır; (7) Türkçe raporda eksik gerekçesi İngilizceydi → yerelleştirildi; (8) aynı sitenin sayfaları (apilex.ai ×4) ayrı kaynak sayıldı → yayıncı başına tek origin (`site_of`) | ✅ |
...
- [x] Örnekler: 4 soru gerçek anahtarlarla (Gemini + Tavily) API üzerinden koşuldu, `examples/<slug>/` (input/trace/report/gate_result/state); bulgular D43'te, düzeltmelerden sonra yeniden üretildi. `examples/offline-demo-eu-ai-act` çevrimdışı format örneği
- [ ] Eşik kalibrasyonu
